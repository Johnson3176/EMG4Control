...
int Filter(int FILTER_N, int pin) {//过滤
...
void adc_read2buf() {//获取5个手指的数值
    static u32 knob_value;
    static int16_t ax_t[5],ay_t[5],az_t[5];
    for(int i=0;i<5;i++) {
        knob_value = 0;
        for(int j=0;j<10;j++) {
            knob_value+=analogRead(dwq_pin[i]);//读模拟引脚，返回0~1023之间的值
...
void loop_Gihand_R(){    
    ...
    if(millis() - systick_ms_bak > 100) {//millis():返回自Arduino开发板开始运行当前程序以来经过的毫秒数
        systick_ms_bak = millis();
        ...
        } else  if((pwm_value[0]<1200) &&(pwm_value[1]>1800) &&(pwm_value[2]>1800) &&(pwm_value[3]<1200) &&(pwm_value[4]<1200) ) {//张开手控制
//            if(abs(pos_x - pos_x_bak) >= 5 || abs(pos_y - pos_y_bak) >= 5) {
//                pos_x_bak = pos_x;
//                pos_x = pos_x-pos_x_verify;
//                pos_x = 1500 - pos_x*4;
